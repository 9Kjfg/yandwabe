...
internal void
DrawRectangleHopefullQuickly(loaded_bitmap *Buffer,
	v2 Origin, v2 XAxis, v2 YAxis, v4 Color,
	loaded_bitmap *Texture,	real32 PixelsToMeters)
{
	BEGIN_TIMED_BLOCK(DrawRectangleHopefullQuickly);
	...
	// TODO: STOP DOING THIS ONCE WE HAVE REAL ROW LOADING
...
#define M(a, i) (((float *)&(a))[i])
			...
			__m128 InvTexelA_4x = _mm_sub_ps(One, Texela);
			Blendedr = _mm_add_ps(_mm_mul_ps(InvTexelA_4x, Destr), Texelr);
			Blendedg = _mm_add_ps(_mm_mul_ps(InvTexelA_4x, Destg), Texelg);
			Blendedb = _mm_add_ps(_mm_mul_ps(InvTexelA_4x, Destb), Texelb);
			Blendeda = _mm_add_ps(_mm_mul_ps(InvTexelA_4x, Desta), Texela);
	...
	END_TIMED_BLOCK(DrawRectangleHopefullQuickly);
}
...
#else
				DrawRectangleHopefullQuickly(OutputTarget, Basis.P,
	...
	uint32 ResolutionPixelsX, uint32 ResolutionPixelsY)
{
	...
	Result->GameCamera.FocalLength = 0.6f; // NOTE: Meters the person is sitting from the monitor
	...
	{
		0.5f*(real32)ResolutionPixelsX*PixelsToMeters,
		0.5f*(real32)ResolutionPixelsY*PixelsToMeters
	};
