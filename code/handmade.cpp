...
AddMonster(game_state *GameState, uint32 AbsTileX, uint32 AbsTileY, uint32 AbsTileZ)
{
	...
	world_position P = ChunkPositionFromTilePosition(GameState->World, AbsTileX, AbsTileY, AbsTileZ);
	add_low_entity_result Entity = AddGroundedEntity(GameState, EntityType_Monster, P, Dim);
	AddFlags(&Entity.Low->Sim, EntityFlag_Collides|EntityFlag_Moveable);
		...
		++EntityIndex, ++Entity)
	{
		if (Entity->Updatable)
		{
			...
			switch (Entity->Type)
			{
				...
				{
					PushRect(&PieceGroup, V2(0, 0), 0, Entity->Dim.XY, V4(1.0f, 0.5f, 0, 1.0f), 0.0f);// 37, 36
					PushRect(&PieceGroup, V2(0, 0), Entity->Dim.Z, Entity->Dim.XY, V4(1.0f, 1.0f, 0, 1.0f), 0.0f);// 37, 36
				} break;
