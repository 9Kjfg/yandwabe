...
AddWall(game_state *GameState, uint32 AbsTileX, uint32 AbsTileY, uint32 AbsTileZ)
{
	...
	Entity.Low->Sim.Height = GameState->World->TileSideInMeters;//1.4f;
...
internal void
SimCameraRegion(game_state *GameState, world_position NewCameraP)
{
	...
	sim_region = BeginSim(SimArena, GameState->World, GameState->CameraP, CameraBounds);
...
//			}
		...
		++HighEntityIndex)
	{
		...
		low_entity *LowEntity = GameState->LowEntities + HighEntity->StoredIndex;
		...
		switch (LowEntity->Type)
		{
			...
			case EntityType_Familiar:
			{
				...
				real32 BobSine = Sin(2.0f*Entity.High->tBob);
