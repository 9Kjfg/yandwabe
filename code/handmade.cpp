...
extern "C" GAME_UPDATE_AND_RENDER(GameUpdateAndRender)
{
	...
	if (!Memory->IsInitialized)
	{
		...
		GameState->StairCollision = MakeSimpleGroundedCollision(GameState,
			GameState->World->TileSideInMeters, 2.0f*GameState->World->TileSideInMeters,
	...
	sim_region *SimRegion = BeginSim(&SimArena, GameState, 
		GameState->World, GameState->CameraP, CameraBounds, Input->dtForFrame);
		...
		++EntityIndex, ++Entity)
	{
		if (Entity->Updatable)
		{
			...
			switch (Entity->Type)
			{
				...
				{
					PushRect(&PieceGroup, V2(0, 0), 0, Entity->WalkableDim, V4(1.0f, 0.5f, 0, 1.0f), 0.0f);// 37, 36
					PushRect(&PieceGroup, V2(0, 0), Entity->WalkableHeight, Entity->WalkableDim, V4(1.0f, 1.0f, 0, 1.0f), 0.0f);// 37, 36
				} break;
				...
				case EntityType_Familiar:
				{
					...
					PushBitmap(&PieceGroup, &GameState->Shadow, V2(0, 0), 0, HeroBitmaps->Align, (0.5f*ShadowAlpha) + 0.2f*BobSine, 0.0f);
					PushBitmap(&PieceGroup, &HeroBitmaps->Head, V2(0, 0), 0.2f*BobSine, HeroBitmaps->Align);
				} break;
