...
AddWall(game_state *GameState, uint32 AbsTileX, uint32 AbsTileY, uint32 AbsTileZ)
{
	...
	Entity.Low->Sim.Dim.Y = GameState->World->TileSideInMeters;//1.4f;
...
AddStair(game_state *GameState, uint32 AbsTileX, uint32 AbsTileY, uint32 AbsTileZ)
{
	...
	Entity.Low->Sim.Dim.Y = 2.0f*GameState->World->TileSideInMeters;//1.4f;
		...
		++EntityIndex, ++Entity)
	{
		if (Entity->Updatable)
		{
			...
			switch (Entity->Type)
			{
				...
				{
					PushRect(&PieceGroup, V2(0, 0), 0, Entity->Dim.XY, V4(1.0f, 1.0f, 0, 1.0f), 0.0f);// 37, 36
				} break;
