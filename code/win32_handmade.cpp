...
{
	uint32 Result = Queue->EntryCount;
	return(Result);
}
...
CompleteAndGetNextWorkQueueEntry(work_queue *Queue, work_queue_entry Completed)
{
	...
	{
		InterlockedIncrement((uint32 volatile *)&Queue->EntryCompletionCount);
	}

	if (Queue->NextEntryToDo < Queue->EntryCount)
	{	
		uint32 Index = InterlockedIncrement((uint32 volatile *)&Queue->NextEntryToDo) - 1;
