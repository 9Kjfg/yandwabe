	...
	if (StoredEvent)
	{
		...
		switch (Event->Type)
		{
			case DebugType_bitmap_id:
			{
				...
				debug_interaction TearInteraction = EventInteraction(DebugState, DebugID, DebugInteraction_TearValue, Event);
			...
			case DebugType_CounterFunctionList:
			{
				...
				DrawProfileIn(DebugState, Element_.Bounds, Layout->MouseP);
			} break;
...
GetGroupForHierarchicalName(debug_state *DebugState, debug_variable_group *Parent, char *Name)
{
	...
	if (FirstSeparator)
	{
		debug_variable_group *SubGroup =
			GetOrCreateGroupWithName(DebugState, Parent, (u32)(FirstSeparator - Name), Name);
...
GetElementFromEvent(debug_state *DebugState, debug_event *Event)
{
	...
	{
		if (StringAreEqual(Chain->GUID, Event->GUID))
		{
			Result = Chain;
			break;
		}
				...
				case DebugType_EndBlock:
				{
					if (Thread->FirstOpenCodeBlock)
					{
						...
						if (EventMatch(*OpeningEvent, *Event))
						{
							...
							{
								char *MatchName = MatchingBlock->Parent ? MatchingBlock->Parent->OpeningEvent->GUID : 0;
