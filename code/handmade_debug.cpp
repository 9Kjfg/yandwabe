	...
	debug_element *RootElement)
{
		...
		Link = Link->Next, ++Index)
	{
		...
		EndDebugStatisctic(&Entry->Stats);
	...
	switch (Element->Type)
	{
		...
		case DebugType_ArenaOccupancy:
		{
			...
			PushRect(&DebugState->RenderGroup, DebugState->BackingTransform,
				LayEl.Bounds, 0.0f, V4(0, 0, 0, 0.5f));
