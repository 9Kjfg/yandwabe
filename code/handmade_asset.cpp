...
MergeIfPossible(game_assets *Assets, asset_memory_block *First, asset_memory_block *Second)
{
	...
	{
		if (!(First->Flag & AssetMemory_Used) &&
			!(Second->Flag & AssetMemory_Used))
		{
...
AcquireAssetMemory(game_assets *Assets, u32 Size, u32 AssetIndex, asset_header_type AssetType)
{
	...
	for (;;)
	{
		...
		{
			Block->Flag |= AssetMemory_Used;

			Result = (asset_memory_header *)(Block + 1);

			memory_index RemainingSize = Block->Size - Size;
			memory_index BlockSplitThreshold = 4096; // TODO: Set this based on the smallest asset
			...
			{
				// TODO; Actually record the unused poriton of the memory
				// in a block so that we can do the merge on blocks when neighborns
				// are freed
			}
		...
		else
		{
				...
				Header = Header->Prev)
			{
				...
				{
					u32 _AssetIndex = Header->AssetIndex;
					asset *Asset = Assets->Assets + _AssetIndex;
					...
					MergeIfPossible(Assets, Block->Prev, Block);
	...
	if (Result)
	{
		Result->AssetType = (u32)AssetType;
...
inline r32
GetGlyphFormCodePoint(hha_font *Info, loaded_font *Font, u32 CodePoint)
{
	u32 Result = 0;
...
{
	u32 PrevGlyph = GetGlyphFormCodePoint(Info, Font, DesiredPrevCodePoint);
	u32 Glyph = GetGlyphFormCodePoint(Info, Font, DesiredCodePoint);
