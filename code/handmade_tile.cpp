...
SetTileValueUnchecked(
    tile_map *TileMap, tile_chunk *TileChunk, uint32 TileX, uint32 TileY,
    uint32 TileValue)
{
	Assert(TileChunk);
	Assert(TileX < TileMap->ChunkDim);
...
inline void
SetTileValue(
	tile_map *TileMap, tile_chunk *TileChunk,
...
IsTileMapPointEmpty(tile_map *TileMap, tile_map_position Pos)
{
	...
	bool32 Empty = IsTileMapEmpty(TileChunkValue);
...
RecanonicalizeCoord(tile_map *TileMap, uint32 *Tile, real32 *TileRel)
{
	...
	// NOTE: TileMap is assumed to ve toroidal topology, if you step off one end you
	...
	*TileRel -= Offset*TileMap->TileSideInMeters;
	...
	// TODO: Fix floating point math  so this ca be <
	Assert(*TileRel >= -0.5f*TileMap->TileSideInMeters);
	Assert(*TileRel <= 0.5f*TileMap->TileSideInMeters);
}

inline tile_map_position
MapIntoTileSpace(tile_map *TileMap, tile_map_position BasePos, v2 Offest)
{
